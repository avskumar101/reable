	...
	if ($_POST['btnsubmit']=="Submit")
	{
		 if($_POST['captcha']!='')
		{
			...
			{ 
				...
				}					if ($_GET['pageflag'] == "custlink")
				{
					$alldeleteids = explode(",",$_GET['deleteids']);
					for($x=0; $x<count($alldeleteids); $x++)
					{
						$sql="UPDATE tbl_custom_tourlink SET delete_status=1 WHERE MLSNo=".$alldeleteids[$x];
						$r=mysql_query($sql);																		$sql1="UPDATE tbl_listings SET cust_url = '' WHERE MLSNo=".$alldeleteids[$x];
						$r1=mysql_query($sql1);
					}
					echo "<script>window.opener.location.href='tourlink.php';window.close();</script>";
				}
