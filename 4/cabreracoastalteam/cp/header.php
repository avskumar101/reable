...
?>
 ...
 .horizontalmenu ul { list-style:none; z-index:20;}
 .horizontalmenu li { height:29px;float:left; position:relative; display:block;font-size:18px;margin-right:1.73em;} 
 .horizontalmenu li ul { display:none; position:absolute;margin-top:6px; } 
 .horizontalmenu li:hover ul{ height:23px; display:block;height:auto; background:rgba(48,108,180,0.8);position:absolute;} 
 .horizontalmenu li ul li{ height:23px; margin-left:-30px; border-style:none;font-size:16px;white-space:nowrap;background:url(../images/dropdownarrow.png) no-repeat left;} 
 .horizontalmenu li ul li a{ color:#fff;text-decoration:none;}
 </style> 
<tr>
    <td><?php echo $headervalue; ?></td></tr>
  <tr>
    <td><table width="1121" border="0" align="center" cellpadding="0" cellspacing="0">
       <tr>
		...
		<?php 
			$resultarrays = mysql_query("SELECT * FROM tbl_web_subpage WHERE main_link ='SALES' and delete_status!=1 order by orderstatus"); 
			$i=1;
        ...
        <ul> 
		<?php 
			$resultarrays = mysql_query("SELECT * FROM tbl_web_subpage WHERE main_link ='CAPE MAY COUNTY' and delete_status!=1 order by orderstatus"); 
			$i=1;
			while($info = @mysql_fetch_array($resultarrays))
			{
			?>
        ...
        <ul> 
		<?php 
			$resultarrays = mysql_query("SELECT * FROM tbl_web_subpage WHERE main_link ='RENTALS' and delete_status!=1 order by orderstatus"); 
			$i=1;
			while($info = @mysql_fetch_array($resultarrays))
			{
			?>
        ...
        <ul> 
		<?php 
			$resultarrays = mysql_query("SELECT * FROM tbl_web_subpage WHERE main_link ='MEET THE TEAM' and delete_status!=1 order by orderstatus"); 
			$i=1;
			while($info = @mysql_fetch_array($resultarrays))
			{
			?>
        ...
        <ul> 
		<?php 
			$resultarrays = mysql_query("SELECT * FROM tbl_web_subpage WHERE main_link ='PROPERTY MANAGEMENT' and delete_status!=1 order by orderstatus"); 
			$i=1;
			while($info = @mysql_fetch_array($resultarrays))
			{
			?>
