<?php
	...
	if(isset($_POST['button'])=="Add Agent"){
		...
		$qry="insert into tbl_listingsagent(agent_firstname,agent_lastname,delete_status) values ('".$agent_firstname."','".$agent_lastname."','0')";
		mysql_query($qry);
    ...
    <td><table width="1122" border="0" align="center" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td><h2>SITE TEMPLATE &gt; AGENTS FOR CABRERA LISTINGS</h2>
          <p>SELECT AN AGENT: 
		  <?php
					$result=@mysql_query("SELECT distinct `agent_firstname`,`agent_lastname`FROM `tbl_listings` WHERE agent_firstname is not null  and agent_lastname is not null and `Org_Name` = 'Long & Foster Real Estate Inc - Wc' ");
			...
			<?php
			 while($resultarray = @mysql_fetch_array($result))
				   {
				   ?>
              <option selected="selected"><?php echo $resultarray['agent_firstname'];?> <?php echo $resultarray['agent_lastname'];?> </option>
            ...
            <tr>
              <td bgcolor="#CCCCCC"><strong>Displaying Properties Listed By:</strong>                <div align="center"></div></td>
			   ...
			   <?php
				   ...
				   ?>
            <tr>
              <td bgcolor="#CAE1F7" class="lrgspacing"><?php echo $resultarray['agent_firstname'];?>(<a href="#" onclick="return eventdelete(<?php echo $resultarray['id']; ?>);">x</a>)<br />
               </td>
			   <?php
			   }
			   ?>
