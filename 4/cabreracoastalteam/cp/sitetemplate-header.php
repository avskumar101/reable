<?php
	...
	if($_POST['uploadfile']=="Upload File")		
	{
		if($_POST['captcha']!='')
		{
			if(captcha_validate())
			{
				$headertext=mysql_real_escape_string(stripcslashes($_POST['header_text']));
				if (mysql_num_rows(mysql_query("select * from tbl_header_text"))<=0)
				{
					
					mysql_query("insert into tbl_header_text(header_text) values ('".$headertext."')");
				}
				else
				{
					mysql_query("update tbl_header_text set header_text='".$headertext."'");
				}
			}
			else
			{
	...
	$opt_row=mysql_fetch_array(mysql_query("select * from tbl_header_text"));
...
<form enctype="multipart/form-data" name="header_texts" id="header_texts" method="POST">
  <tr>
  ...
  <tr>
    <td><table width="1122" border="0" align="center" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td><h2>SITE TEMPLATE &gt; HEADER</h2>
          <p>
            <textarea rows="15" style="width: 98%" name="header_text" id="header_text"><?php echo $opt_row['header_text'];?></textarea>
          </p>
