	...
	if($_POST['btn_person']=="Update")		
	{
		...
		{ 
			if(captcha_validate())
			{
					...
					$original_name=$_POST['hidefile'];
					}
					$qry="UPDATE  tbl_team_person SET upload_picture='$name',image_filename='$original_name',name='$personname',title='$title',email='$email',phone='$phone',description='$descriptions' where id='$ID'";
          ...
          <table width="1122" border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <td>&nbsp;</td>
			   ...
			   <?php $resultarrays = mysql_fetch_array(mysql_query("SELECT * FROM  tbl_team_person WHERE id ='".$ID."'")); 
		 
		  ?>
            <tr>
				 <input type="hidden" name="hidefile" id="hidefile" value="<? echo $resultarrays['image_filename']; ?>" size="2" />
				 <?
					if($_POST['person_name']=="")
						$txt_name_value=$resultarrays['name'];
					else
						$txt_name_value=$_POST['person_name'];
					?>
              <td colspan="4"><img src="../images/t.gif" width="10" height="4" /></td>
            </tr>
            <tr>
              <td><input name="person_name" type="text" id="person_name" style="width: 85%" value="<?php echo $txt_name_value;?>" /></td>
			  <?
					if($_POST['person_title']=="")
						$txt_title_value=$resultarrays['title'];
					else
						$txt_title_value=$_POST['person_title'];
					?>
              <td width="281"><input name="person_title" type="text" id="person_title" style="width: 85%" value="<?php echo $txt_title_value; ?>"  /></td>
			  <?
					if($_POST['person_email']=="")
						$txt_email_value=$resultarrays['email'];
					else
						$txt_email_value=$_POST['person_email'];
					?>
               <td width="281"><input name="person_email" type="text" id="person_email" style="width: 85%" value="<?php echo $txt_email_value;?>" /></td>
			    <?
					if($_POST['person_phone']=="")
						$txt_email_value=$resultarrays['phone'];
					else
						$txt_phone_value=$_POST['person_email'];
					?>
               <td width="281"><input name="person_phone" type="text" id="person_phone" style="width: 85%" value="<?php echo $resultarrays['phone'];;?>" /></td>
              <td width="280">&nbsp;</td>
            </tr>
            <tr>
              <td colspan="5">&nbsp;</td>
            ...
            <tr>
              <td colspan="5"><input type="file" accept=".png,.jpg,.jpeg" onchange="ValidateSingleInput(this);" name="fileField" id="fileField"  /><?php echo $resultarrays['image_filename']; ?></td>
            </tr>
            <tr>
              <td colspan="5">&nbsp;</td>
            </tr>
            ...
            <tr>
			 ...
			 <?
					if($_POST['description']=="")
						$txt_description_value=$resultarrays['description'];
					else
						$txt_description_value=$_POST['description'];
					?>
            <tr>
               <td colspan="5"><textarea name="description" rows="15" id="description" style="width: 98%"><?php echo stripslashes($txt_description_value);?></textarea></td>
            </tr>
            <tr>
              <td colspan="5">&nbsp;</td>
            </tr>
            <tr>
                ...
                <tr>
                  <td><input type="submit" name="btn_person" id="btn_person" value="Update" />
                    <input type="button" name="button4" id="button5" value="Cancel" onclick="window.location.href='updatepages-ourteam.php'" /></td>
                </tr>
              </table></td>
            </tr>
          </table></td>
