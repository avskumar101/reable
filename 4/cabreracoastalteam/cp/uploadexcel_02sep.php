...
$exceldata[$i]=PHPExcel_Style_NumberFormat::toFormattedString($cell->getValue(), "M/D/YYYY");
		...
		for($ij=1;$ij<$rowcount;$ij++) {
				$excelrow=$sheetarray[$ij];
				 ...
				 mysql_query("insert into tbl_sold(date,market,address,city,style,mls_no,asking_price,soldprice,city_value) values('".$SoldDate."','".$DaysOnMarket."','".$Address."','".$City."','".$Type."','".$MLS."','".$AskingPrice."','".$SoldPrice."','".$citytypeval."')");
			}
	//$sheetarray[$rowcount] = $exceldata;
			//print_r($sheetarray[$rowcount]);
