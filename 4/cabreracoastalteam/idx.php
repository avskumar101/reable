		...
		foreach ($classes as $classval)
		{
			...
			{				
				...
				else if ($listing['L_Class']=='COMMERCIAL/INDUSTRIAL')
				{
					...
					$Lot_Size1='';
					$Lot_Size2=$listing['LM_Char10_2'];
					$Lot_Number=$listing['LM_Char10_3'];										$bankown=$listing['LM_Char10_21'];					
					$Number_Of_Acres='';
					$New_Construction=$listing['LM_Char10_19'];
					$Possession=$listing['LM_Char25_1'];
					$Appx_Living_Square_Feet=$listing['L_SquareFeet'];
				...
				else if ($listing['L_Class']=='CONDO/TOWNHOUSE')
				{
					...
					$City=$listing['L_City'];
					$State=$listing['L_State'];
					$Zip=$listing['L_Zip'];					$bankown=$listing['LM_Char10_21'];					
					$Original_Price=$listing['L_OriginalPrice'];
					$Asking_Price=$listing['L_AskingPrice'];
					$Year_Built=$listing['LM_Char10_1'];
					$Bedrooms=$listing['L_Keyword1'];
				...
				$OffMktDate=date('Y-m-d',strtotime($listing['L_UpdateDate']));
				
				$Tax_Year=$listing['LM_Char10_10'];
				
				if (mysql_num_rows(mysql_query("select * from tbl_listings where MLSNo='".$MLSNo."'"))>0)
				{
					$insqry="update tbl_listings set 
				...
				else
				{
					...
					'".mysql_real_escape_string($Tax_Year)."',1,'".mysql_real_escape_string($L_ListOffice)."','".mysql_real_escape_string($listuserfirstname)."','".mysql_real_escape_string($listuserlastname)."','".mysql_real_escape_string($tour_url)."','".mysql_real_escape_string($bankown)."')";
				}				
